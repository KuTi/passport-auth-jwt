...
function JwtStrategy(options, verify) {
	...
	if (!this._secret) {
		throw new TypeError('[Passport Auth JWT] requires a secret');
	}
...
JwtStrategy.prototype.authenticate = function (req, options) {
	...
	if (!token) {
		return self.fail(new Error("Can't get Token."));
	}

	// Verify the JWT
	JwtStrategy.JwtVerifier(token, this._secret, this._verifOpts, function (jwt_err, payload) {
		if (jwt_err) {
