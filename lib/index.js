...
PassportAuthJWT.prototype = {
	_config: {
		...
		hook: {
			...
			generateRefreshToken: this._generateRefreshToken,
	...
	authenticate: function (opts) {
		...
		return function (req, res, next) {
			//if (req.headers['apiVersion'] === 4) {
				...
				if (self._config.ignoreRequestTokenUrl.indexOf(path) === -1) {
					switch (path) {
						...
						case self._config.refreshTokenEndpoint:
							passport.authenticate(
								...
								self._config.hook.generateRefreshToken(req, res, next, self._config)
							)(req, res, next);
							break;
	...
	_generateRefreshToken: function () {
		console.log('[Passport Auth JWT] "hook.generateRefreshToken," must be a function in the options of Passport Auth JWT');
	},
